...
#     self.writeRegister(ILI9225_RAM_ADDR_SET2,b'\x00\x00')
  ...
  def char(self, char, x, y, color=0xffff, background=0x0000):
      ...
      data = bytearray([])  #1 is the bit width
      data_index=0
...
# bitmaps routine
  ...
  def DrawMarker(self,x,y):    
      """color is an 16 bit RGB integer value"""
      #self.setWindow(x1-4,y1-8,x1+4,y1)
      x1=x-4
      y1=y-8
      self.setWindow(x1,y1,x1+7,y1+7)
      mark_buff=b'\xff\xff\xff\xff\xfc\x92\xf8\x00\xf8\x00\xfc\x92\xff\xff\xff\xff\xff\xff\xfdu\xf8\x00\xf9e\xf9e\xf8\x00\xfdu\xff\xff\xff\xff\xfaI\xf8\x00\xff\xff\xff\xff\xf8\x00\xfaI\xff\xff\xff\xff\xfb\x0c\xf8\x00\xfc\xf3\xfc\xf3\xf8\x00\xfb\x0c\xff\xff\xff\xff\xfe8\xf8\x00\xf8\x00\xf8\x00\xf8\x00\xfe8\xff\xff\xff\xff\xff\xff\xf8a\xf8\x00\xf8\x00\xf8a\xff\xff\xff\xff\xff\xff\xff\xff\xff<\xf8\x00\xf8\x00\xff<\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xfc\xb2\xfc\xb2\xff\xff\xff\xff\xff\xff'
      # len(imageArray)
      self.write_cmd(ILI9225_GRAM_DATA_REG)
      self.rs.value(1)
      self.cs.value(0)
      self.spi.write(mark_buff)
      self.cs.value(0)

         
  def StatDisplay(self,background=0x0000):
        self.clear()
        self.text('Location',2,5,0x07E0,background)
        self.text('Info',115,5,0x07E0,background)
        self.text('Lat:',5,30,0xFFFF,background)
        self.text('Lon:',5,52,0xFFFF,background)
        self.text('FMA:',5,82,0xFFFF,background)   
        self.text('LGU:',5,102,0XFFFF,background)  
        self.text('9',70,82,0xFFE0,background)  #FMA update
        self.text('El Salv',70,102,0xFFE0,background) #LGU update
        self.text('DiO:',5,122,0XFFFF,background)
        self.text('DiL:',5,142,0XFFFF,background)
        self.text('Waters:',5,162,0XFFFF,background)
        self.text('Time:',5,182,0XFFFF,background)
        self.text('Mun',120,162,0XFFFF,background)  #water update
        self.text('NumSat:',5,200,0XFFFF,background)
               
  def LocUpdate(self,lat,long,time,num_sat,d,d0,background=0x0000):
        self.text("{:7.4f}".format(lat),60,30,0xFFFF,background)
        self.text("{:7.4f}".format(long),60,52,0xFFFF,background)
        self.text(str(time),65,182,0xFFE0, background)
        self.text("{:02d}".format(num_sat),100,200,0xFFE0,background)
        self.text("{:06.3f}".format(d)+'  ',70,142,0xFFE0,background) #distLGU update
        self.text("{:06.3f}".format(d0)+'  ',70,122,0xFFE0,background) #disOrg update
      #self.resetWindow()
