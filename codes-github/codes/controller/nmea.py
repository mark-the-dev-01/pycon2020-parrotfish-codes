...
class nmea():
	def __init__(self, debug=0):
		self._sentence = ''
		self.time = '00:00:00'
		self.time_local='00:00:00' #local time
	...
	def format(self):
		...
		if data[0] == 'GPRMC':
			...
			try:
				...
				self.time_local='{:2d}:{}:{}'.format(int(hh)+8, mm, ss) #philippine time=utc+8:00
			except Exception as e:
				self.dprint('time error')
				return
...
def get_distance(lat_1, lng_1, lat_2, lng_2): # in Km
